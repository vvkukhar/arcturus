  ...
  return (
    <div className="flex flex-col space-y-8 rounded-[2rem] border border-[var(--border)] bg-[var(--card)] p-6 md:p-8 shadow-sm">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-black text-[var(--foreground)] tracking-tight">{t('admin.scanner.control' as any)}</h2>
      ...
      <form onSubmit={handleAddSource} className="space-y-4 bg-[var(--background)]/50 p-5 rounded-2xl border border-[var(--border)]">
        <h3 className="text-xs font-black uppercase tracking-widest text-slate-500">{t('admin.scanner.addSource' as any)}</h3>
        <div className="grid gap-3 md:grid-cols-2">
          <input required value={code} onChange={(e) => setCode(e.target.value)} placeholder={t('admin.scanner.sourceCode' as any)} className="w-full rounded-xl border border-[var(--border)] bg-[var(--card)] px-4 py-3 text-sm font-medium focus:border-blue-500 outline-none text-[var(--foreground)]" />
          <input required value={name} onChange={(e) => setName(e.target.value)} placeholder={t('admin.scanner.displayName' as any)} className="w-full rounded-xl border border-[var(--border)] bg-[var(--card)] px-4 py-3 text-sm font-medium focus:border-blue-500 outline-none text-[var(--foreground)]" />
        </div>
        <button type="submit" disabled={loading === 'source' || !code.trim() || !name.trim()} className="flex w-full sm:w-auto items-center justify-center gap-2 rounded-xl bg-blue-600 px-8 py-3 text-sm font-bold text-white hover:bg-blue-700 disabled:opacity-50">
          ...
          {loading === 'source' ? t('common.loading' as any) : t('common.add' as any)}
        </button>
      </form>

      <form onSubmit={handleEnqueueScan} className="space-y-4 bg-blue-500/10 p-5 rounded-2xl border border-blue-500/20">
        <h3 className="text-xs font-black uppercase tracking-widest text-blue-500">{t('admin.scanner.enqueueJob' as any)}</h3>
        <div className="grid gap-3 md:grid-cols-2">
          <select required value={sourceCode} onChange={(e) => setSourceCode(e.target.value)} className="w-full rounded-xl border border-[var(--border)] bg-[var(--card)] px-4 py-3 text-sm font-bold text-[var(--foreground)] focus:border-blue-500 outline-none cursor-pointer">
            <option value="" disabled>{t('admin.scanner.selectSource' as any)}</option>
            {sources.map((source) => <option key={source.code} value={source.code}>{source.name}</option>)}
          </select>
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={t('admin.scanner.searchQuery' as any)} className="w-full rounded-xl border border-[var(--border)] bg-[var(--card)] px-4 py-3 text-sm font-medium focus:border-blue-500 outline-none text-[var(--foreground)]" />
        </div>
        <button type="submit" disabled={loading === 'job' || !sourceCode} className="flex w-full sm:w-auto items-center justify-center gap-2 rounded-xl bg-blue-600 px-8 py-3 text-sm font-bold text-white hover:bg-blue-700 disabled:opacity-50">
          ...
          {loading === 'job' ? t('common.loading' as any) : t('admin.scanner.enqueueJob' as any)}
        </button>
      </form>
    </div>
  );
