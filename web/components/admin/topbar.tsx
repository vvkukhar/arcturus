  ...
  return (
    <>
      ...
      <div className="sticky top-0 z-40 flex items-center justify-between border-b border-[var(--border)] bg-[var(--card)]/80 backdrop-blur-xl px-6 py-4 transition-colors duration-300">
        <div>
          <div className="text-lg font-black text-[var(--foreground)]">{t('admin.workspace' as any) as string}</div>
          <div className="flex items-center gap-2 mt-1">
            <span className="text-[10px] font-bold uppercase tracking-widest text-slate-500">{t('admin.quickActions' as any) as string}:</span>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          >
            <RefreshCcw size={16} />
            <span className="hidden sm:inline">{t('admin.refresh' as any) as string}</span>
          </Button>
          ...
          >
            <LogOut size={16} />
            <span className="hidden sm:inline">{t('admin.logout' as any) as string}</span>
          </Button>
        </div>
      </div>
    </>
