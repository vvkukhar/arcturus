  ...
  return (
    <div className="p-6 md:p-10 max-w-5xl mx-auto animate-fade-in-up">
      <div className="mb-10 flex flex-col md:flex-row justify-between md:items-end gap-4">
        <div>
          <h1 className="text-3xl md:text-5xl font-black tracking-tight">{t('orderbook.title')}</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-2 text-lg font-medium">
            {t('orderbook.subtitle').replace('{title}', itemContext?.titleSnapshot || t('orderbook.defaultAsset'))}
          </p>
        </div>
        <div className="px-4 py-2 bg-[var(--card)] border border-[var(--border)] rounded-xl flex items-center gap-2 font-bold text-sm shadow-sm">
          <Activity size={16} className="text-blue-500" /> {t('orderbook.live')}
        </div>
      </div>
      ...
      <div className="bg-[var(--card)] rounded-3xl border border-[var(--border)] shadow-sm overflow-hidden flex flex-col md:flex-row">
        <div className="flex-1 border-r border-[var(--border)]">
          <div className="p-4 border-b border-[var(--border)] bg-slate-50/50 dark:bg-slate-900/50">
            <h3 className="font-bold text-center text-red-500">{t('orderbook.asks')}</h3>
          </div>
          <table className="w-full text-right text-sm font-medium">
            <thead>
              <tr className="text-slate-500 text-xs">
                <th className="p-3">{t('orderbook.price')}</th>
                <th className="p-3">{t('orderbook.size')}</th>
                <th className="p-3">{t('orderbook.total')}</th>
              </tr>
            </thead>
            <tbody>
              {asks.length > 0 ? asks.map((ask, i) => (
                ...
                <tr><td colSpan={3} className="p-6 text-center text-slate-400">{t('orderbook.noAsks')}</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="flex-1">
          <div className="p-4 border-b border-[var(--border)] bg-slate-50/50 dark:bg-slate-900/50">
            <h3 className="font-bold text-center text-green-500">{t('orderbook.bids')}</h3>
          </div>
          <table className="w-full text-right text-sm font-medium">
            <thead>
              <tr className="text-slate-500 text-xs">
            ...
            <tbody>
              {bids.length > 0 ? bids.map((bid, i) => (
                ...
                <tr><td colSpan={3} className="p-6 text-center text-slate-400">{t('orderbook.noBids')}</td></tr>
              )}
            </tbody>
          </table>
        </div>
