...
export default function ScoutPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Ціль захоплено! Аналітики перевірять лінк.');
      setUrl('');
      setNotes('');
      mutate();
    } catch (err: any) {
    ...
    <main className="min-h-screen py-16 px-4 bg-[var(--background)]">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="text-center animate-in fade-in slide-in-from-bottom-4 duration-500 relative">
          ...
          <p className="text-lg text-slate-500 font-medium">Знаходьте недооцінені набори. Ми їх викупуємо — ви отримуєте Arcturus Credits (AC). Обмінюйте на знижки та Mystery Boxes.</p>
        ...
        {user && (
          <div className="bg-slate-900 dark:bg-black p-8 rounded-[2.5rem] border border-slate-800 shadow-2xl relative overflow-hidden text-white animate-in fade-in">
            ...
            <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
              <div>
                <div className="text-xs font-black uppercase tracking-widest text-slate-400 mb-1">Ваш Ранг</div>
              ...
              <div className="flex-1 w-full max-w-sm">
                ...
                <div className="text-center mt-2 text-xs font-medium text-slate-500">До наступного рангу: {nextRankPoints - points} AC</div>
              </div>
              <div className="text-center shrink-0">
                <div className="text-xs font-black uppercase tracking-widest text-slate-400 mb-1">Баланс</div>
                <div className="text-4xl font-black text-emerald-400">{points} <span className="text-sm">AC</span></div>
                <Link href="/scout/rewards" className="text-xs font-bold text-blue-400 hover:text-blue-300 mt-2 block underline">Витратити бали</Link>
              </div>
            </div>
          </div>
        )}
        ...
        {safeSurges.length > 0 && (
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-900/50 p-6 rounded-[2rem] animate-in fade-in slide-in-from-bottom-4 shadow-sm">
            <div className="flex items-center gap-2 text-red-600 dark:text-red-400 font-black uppercase tracking-widest mb-4">
              <Flame size={20} className="animate-pulse" /> Гарячий попит (Більше AC)
            </div>
