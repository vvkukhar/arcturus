...
export default function ScreenerPage() {
  ...
  // ФІКС: Тягнемо дані з PRO ендпоінту
  const { data: rawData, isLoading } = useSWR<ScreenerItem[]>('/api/proxy/pro/screener', swrFetcher);
  ...
  const filtered = useMemo(() => {
    ...
    result.sort((a, b) => {
      const aTarget = a.expectedSalePriceManual ?? 0;
      const bTarget = b.expectedSalePriceManual ?? 0;
      const aRoi = a.roiPercent ?? 0;
      const bRoi = b.roiPercent ?? 0;
  ...
  const exportCSV = () => {
    const headers = ['Set ID', 'Name', 'Theme', 'Market Target', 'Est. ROI'];
        ...
        <div className="bg-[var(--card)] p-4 rounded-t-2xl border border-[var(--border)] shadow-sm flex flex-wrap items-center gap-4">
          <div className="relative flex-1 min-w-[250px]">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-[var(--border)] rounded-xl focus:ring-2 focus:ring-blue-600 outline-none text-sm font-medium text-[var(--foreground)]" 
            />
          </div>
          <select 
            value={themeFilter}
            onChange={(e) => setThemeFilter(e.target.value)}
            className="bg-slate-50 dark:bg-slate-950 border-none text-sm font-bold rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-blue-600 cursor-pointer transition-colors text-[var(--foreground)]"
          >
          ...
          <button onClick={() => { setQuery(''); setThemeFilter(''); setSort('roi_desc'); }} className="flex items-center gap-2 px-4 py-2.5 bg-slate-900 text-white rounded-xl font-bold text-sm shadow-md hover:bg-black transition-colors dark:bg-white dark:text-slate-900 dark:hover:bg-slate-200">
                ...
                {filtered.map((row) => {
                  return (
                    <tr key={row.id} className="border-b border-[var(--border)] hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors">
                      ...
                      <td className="p-4 font-bold">{formatMoney(row.expectedSalePriceManual ?? 0)}</td>
                      <td className={`p-4 text-right font-black ${row.roiPercent >= 0 ? 'text-green-500' : 'text-red-500'}`}>
