...
export default function CheckoutPage() {
    ...
    try {
      ...
      const reserveResponse = await apiFetch<ApiResponse<any>>('/api/store/contact', {
        ...
        body: JSON.stringify({
          ...
          productTitle: items.length > 1 ? t('checkout.multiItem').replace('{count}', String(items.length)) : items[0].title,
          name,
  ...
  if (items.length === 0) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-20 md:py-32 text-center">
        ...
        <h1 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white mb-4">{t('checkout.emptyTitle')}</h1>
        <p className="text-sm md:text-base text-slate-500 dark:text-slate-400 mb-8">{t('checkout.emptyDesc')}</p>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 animate-fade-in-up">
      <h1 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white mb-6 md:mb-8">{t('checkout.title')}</h1>
      ...
      <div className="flex flex-col-reverse lg:flex-row gap-8 lg:gap-12">
          ...
          <form id="checkout-form" onSubmit={handleCheckout} className="space-y-6 md:space-y-8">
            <div className="bg-white dark:bg-slate-950 p-5 sm:p-6 md:p-8 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
              <h2 className="text-lg md:text-xl font-black text-slate-900 dark:text-white mb-4 md:mb-6 flex items-center gap-2">
                <ShieldCheck size={20} className="text-blue-600" /> {t('checkout.contactInfo')}
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs md:text-sm font-bold text-slate-700 dark:text-slate-300 mb-1.5">{t('checkout.firstName')}</label>
                ...
                <div>
                  <label className="block text-xs md:text-sm font-bold text-slate-700 dark:text-slate-300 mb-1.5">{t('checkout.lastName')}</label>
                  <input required name="lastName" value={formData.lastName} onChange={handleChange} type="text" className="w-full p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl focus:ring-2 focus:ring-blue-600 outline-none text-slate-900 dark:text-white text-sm md:text-base transition-shadow" />
                </div>
                <div className="sm:col-span-2">
                  <label className="block text-xs md:text-sm font-bold text-slate-700 dark:text-slate-300 mb-1.5">{t('checkout.phone')}</label>
            ...
            <div className="bg-white dark:bg-slate-950 p-5 sm:p-6 md:p-8 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
              <h2 className="text-lg md:text-xl font-black text-slate-900 dark:text-white mb-4 md:mb-6 flex items-center gap-2">
                <MapPin size={20} className="text-blue-600" /> {t('checkout.delivery')}
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs md:text-sm font-bold text-slate-700 dark:text-slate-300 mb-1.5">{t('checkout.city')}</label>
                ...
                <div>
                  <label className="block text-xs md:text-sm font-bold text-slate-700 dark:text-slate-300 mb-1.5">{t('checkout.branch')}</label>
        ...
        <div className="w-full lg:w-[400px] xl:w-[450px]">
          <div className="bg-slate-50 dark:bg-slate-900 p-5 sm:p-6 md:p-8 rounded-2xl border border-slate-200 dark:border-slate-800 lg:sticky lg:top-24">
            <h2 className="text-lg md:text-xl font-black text-slate-900 dark:text-white mb-4 md:mb-6">{t('checkout.orderSummary')}</h2>
              ...
              {items.map(item => (
                <div key={item.id} className="flex gap-3 md:gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs md:text-sm text-slate-500 dark:text-slate-400 mt-1 font-medium">{t('checkout.qty')}: {item.quantity} × {new Intl.NumberFormat('uk-UA', { style: 'currency', currency: 'UAH', maximumFractionDigits: 0 }).format(item.price)}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="border-t border-slate-200 dark:border-slate-800 pt-4 mb-4 md:mb-6">
              <div className="flex justify-between items-center mb-2">
                <span className="text-sm md:text-base text-slate-500 dark:text-slate-400 font-medium">{t('checkout.subtotal')}</span>
              ...
              <div className="flex justify-between items-center">
                <span className="text-sm md:text-base text-slate-500 dark:text-slate-400 font-medium">{t('nav.delivery')}</span>
                <span className="text-sm md:text-base font-bold text-slate-900 dark:text-white">{t('checkout.deliveryCalc')}</span>
              </div>
            </div>

            <div className="flex justify-between items-center mb-6 md:mb-8">
              <span className="text-base md:text-lg font-black text-slate-900 dark:text-white">{t('checkout.total')}</span>
            ...
            <div className="hidden lg:block">
              ...
              <p className="text-xs text-center text-slate-500 dark:text-slate-400 mt-4 font-medium">{t('checkout.securePay')}</p>
            </div>
          </div>
        </div>
      </div>
