...
export default function SellPage() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      const combined = [...files, ...newFiles].slice(0, 5); // Макс 5 фотографій
      setFiles(combined);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Якщо є фотографії - завантажуємо їх
      if (files.length > 0 && createdItem?.id) {
        ...
        await apiFetch(`/api/proxy/marketplace/${createdItem.id}/upload`, {
          method: 'POST',
          body: uploadData // Передаємо FormData напряму
        });
      }
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {previewUrls.map((url, i) => (
                      <div key={i} className="relative aspect-square rounded-2xl border border-[var(--border)] overflow-hidden group">
