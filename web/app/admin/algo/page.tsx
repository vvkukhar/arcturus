...
export default function AlgoControlPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(t('admin.algo.success' as any));
    } catch (err) {
      toast.error(err instanceof Error ? err.message : t('common.error' as any));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[var(--card)] border border-[var(--border)] p-6 md:p-8 rounded-[2rem] shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black text-[var(--foreground)] tracking-tight">{t('admin.algo.title' as any)}</h1>
            <p className="mt-1 text-sm font-medium text-slate-500">{t('admin.algo.subtitle' as any)}</p>
          </div>
        </div>
        <div className="flex items-center gap-3 bg-[var(--background)] px-5 py-3 rounded-2xl border border-[var(--border)]">
          ...
          <span className="font-bold text-sm uppercase tracking-wider">{isAutoPilotActive ? t('admin.algo.autoPilot' as any) : t('admin.algo.manualOverride' as any)}</span>
          <button 
            onClick={() => setIsAutoPilotActive(!isAutoPilotActive)}
            className="ml-4 text-xs font-black text-blue-600 hover:underline"
          >
            {t('admin.algo.toggle' as any)}
          </button>
        </div>
      </div>

      <div className="grid gap-6 xl:grid-cols-2">
        <SectionCard title={t('admin.algo.pricingEngine' as any)}>
          <div className="space-y-8">
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <label className="text-sm font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">{t('admin.algo.aggressiveness' as any)}</label>
              ...
              />
              <p className="text-xs text-slate-500 font-medium">{t('admin.algo.aggressivenessDesc' as any)}</p>
            </div>

            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <label className="text-sm font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">{t('admin.algo.roiCeiling' as any)}</label>
              ...
              />
              <p className="text-xs text-slate-500 font-medium">{t('admin.algo.roiCeilingDesc' as any)}</p>
            </div>
          </div>
        </SectionCard>

        <SectionCard title={t('admin.algo.procurement' as any)}>
          <div className="space-y-8">
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <label className="text-sm font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">{t('admin.algo.capitalLimit' as any)}</label>
              ...
              />
              <p className="text-xs text-slate-500 font-medium">{t('admin.algo.capitalLimitDesc' as any)}</p>
            </div>
            ...
            <div className="p-5 rounded-2xl bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-900/50 flex gap-4">
              ...
              <div>
                <h4 className="font-bold text-amber-800 dark:text-amber-300">{t('admin.algo.riskMgmt' as any)}</h4>
                <p className="text-xs text-amber-700/80 dark:text-amber-400/80 mt-1">{t('admin.algo.riskMgmtDesc' as any)}</p>
              </div>
            </div>
          </div>
        </SectionCard>
      ...
      <div className="flex justify-end pt-4">
        <Button size="lg" onClick={handleSave} disabled={loading} className="gap-2 rounded-[1rem] px-10">
          ...
          {loading ? t('common.loading' as any) : t('admin.algo.deploy' as any)}
        </Button>
      </div>
    </div>
  );
