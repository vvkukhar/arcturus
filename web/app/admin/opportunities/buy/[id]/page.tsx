...
export default function BuyOpportunityDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const { data, isLoading } = useSWR<OpportunityItem>(`/api/admin/opportunities/buy/${id}`, swrFetcher);
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto pb-20 animate-fade-in-up">
      <div>
        <h1 className="text-3xl font-black text-[var(--foreground)]">Opportunity Details</h1>
