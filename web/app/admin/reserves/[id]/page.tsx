...
export default function ReserveDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const { data, isLoading } = useSWR<any>(`/api/admin/public/reserve-requests/${id}`, swrFetcher as any);

  if (isLoading) return <div className="p-10 flex justify-center"><Loader2 className="animate-spin w-8 h-8 text-blue-500" /></div>;
  if (!data) return <div className="p-10 text-center font-bold text-slate-500">Reserve request not found</div>;

  return (
    <div className="space-y-6 max-w-5xl mx-auto pb-20 animate-fade-in-up">
      <div>
        <h1 className="text-3xl font-black text-[var(--foreground)]">Reserve Request Details</h1>
        <p className="mt-1 text-sm text-slate-500 font-mono">ID: {id}</p>
      </div>
