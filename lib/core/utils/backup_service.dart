...
class BackupService {
  static Future<void> createFullBackup() async {
    final payload = {
      'inventory': InventoryRepository().getAllItems().map((e) => e.toMap()).toList(),
      'purchases': PurchasesRepository().getAllPurchases().map((e) => e.toMap()).toList(),
      'sales': SalesRepository().getAllSales().map((e) => e.toMap()).toList(),
      'watchlist': WatchlistRepository().getAll().map((e) => e.toMap()).toList(),
      'market': MarketRepository().getAll().map((e) => e.toMap()).toList(),
      'partoutProjects': PartOutRepository().getAllProjects().map((e) => e.toMap()).toList(),
