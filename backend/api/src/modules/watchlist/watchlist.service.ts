...
export class WatchlistService {
  ...
  async update(id: string, data: Partial<UpdateWatchlistItemDto>): Promise<unknown> {
    ...
    const updated = await this.prisma.watchlistItem.update({
      ...
      data: {
        desiredBuyPrice:
          data.desiredBuyPrice != null ? toMoney(data.desiredBuyPrice) : undefined,
        maxBuyPrice:
          data.maxBuyPrice != null ? toMoney(data.maxBuyPrice) : undefined,
        targetSellPrice:
          data.targetSellPrice === undefined
            ? undefined
            : data.targetSellPrice === null
              ? null
              : toMoney(data.targetSellPrice),
        active: data.active,
    ...
    await this.activity.log('watchlist.updated', {
  ...
  async delete(id: string): Promise<unknown> {
    ...
    const deleted = await this.prisma.watchlistItem.delete({
      where: {
        id,
      },
    });

    await this.activity.log('watchlist.deleted', {
      watchlistItemId: id,
      itemId: existing.itemId,
      title: existing.titleSnapshot,
    });

    this.realtime.emitWatchlistRefresh({
      id,
      deleted: true,
    });
  ...
  async bulkActivate(ids: string[], active: boolean): Promise<unknown> {
    ...
    const result = await this.prisma.watchlistItem.updateMany({
      where: {
        id: {
          in: safeIds,
        },
    ...
    await this.activity.log('watchlist.bulk_activate', {
      ids: safeIds,
      active,
      count: result.count,
    });
  ...
  async bulkDelete(ids: string[]): Promise<unknown> {
    ...
    await this.activity.log('watchlist.bulk_delete', {
      ids: safeIds,
      count: result.count,
    });
  ...
  async stats(): Promise<unknown> {
    ...
    return {
      ...
      highPriority: rows.filter((row) => row.priority >= 75).length,
      assigned: rows.filter((row) => row.assignedUserId != null).length,
    };
  }
}