...
export class InventoryService {
    ...
    const created = await this.prisma.inventoryItem.create({
      data: {
        ...
        expectedSalePriceManual:
          dto.expectedSalePriceManual != null
            ? toMoney(dto.expectedSalePriceManual)
            : null,
  ...
  async update(dto: UpdateInventoryItemDto): Promise<unknown> {
    ...
    const updated = await this.prisma.inventoryItem.update({
      ...
      data: {
        ...
        purchasePrice:
          dto.purchasePrice != null ? toMoney(dto.purchasePrice) : undefined,
        totalCost: dto.totalCost != null ? toMoney(dto.totalCost) : undefined,
        quantity: dto.quantity,
        condition: dto.condition,
        sealed: dto.sealed,
        expectedSalePriceManual:
          dto.expectedSalePriceManual === undefined
            ? undefined
            : dto.expectedSalePriceManual === null
              ? null
              : toMoney(dto.expectedSalePriceManual),
  ...
  async delete(id: string): Promise<unknown> {
    ...
    const deleted = await this.prisma.inventoryItem.delete({
      where: {
        id,
      },
    });
  ...
  async bulkDelete(ids: string[]): Promise<unknown> {
    ...
    await this.activity.log('inventory.bulk_deleted', {
      ids: safeIds,
      count: result.count,
    });
  ...
  async stats(): Promise<unknown> {
    ...
    const totalCost = toMoney(rows.reduce((sum, row) => sum + row.totalCost, 0));
