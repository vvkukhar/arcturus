...
export class PortfolioService {
  ...
  async getInventoryValue(): Promise<unknown> {
    ...
    return {
      ...
      expectedRoi:
        totalCostBasis > 0
          ? Number(((expectedProfit / totalCostBasis) * 100).toFixed(2))
          : 0,
    };
  }
  ...
  async getCapitalPlan(capital: number): Promise<unknown> {
    if (!Number.isFinite(capital) || capital <= 0) {
      return {
        capital,
        allocations: [],
        message: 'Capital must be greater than zero',
      };
    }
    ...
    for (const item of watchlist) {
      ...
      const roiPercent =
        buyPrice > 0
          ? Number(((expectedNetProfit / buyPrice) * 100).toFixed(2))
          : 0;
    ...
    return {
      ...
      allocations,
      reservedCapital: toMoney(
        allocations.reduce((sum, item) => sum + item.reservedCapital, 0),
      ),
      remainingCapital: toMoney(
        capital -
          allocations.reduce((sum, item) => sum + item.reservedCapital, 0),
      ),
    };
