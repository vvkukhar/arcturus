...
export class SalesService {
  ...
  async registerSale(params: {
    ...
    const unitCost =
      inventoryItem.quantity > 0
        ? inventoryItem.totalCost / inventoryItem.quantity
  ...
  async stats(): Promise<{
    ...
    const avgRoi =
      sales.length > 0
        ? toMoney(
            sales.reduce((sum, sale) => sum + Number(sale.roiPercent ?? 0), 0) /
              sales.length,
          )
        : 0;
  ...
  async deleteSale(id: string): Promise<unknown> {
    ...
    if (existing.returns.length > 0) {
      throw new BadRequestException('Cannot delete sale with return requests');
    }

    if (existing.expenses.length > 0) {
      throw new BadRequestException('Cannot delete sale with linked expenses');
    }
    ...
    const deleted = await this.prisma.$transaction(async (tx) => {
      ...
      await tx.stockMovement.create({
        data: {
          ...
          reason: `Sale deleted ${existing.id}`,
        },
      });
