...
export class WarehouseService {
  ...
  async createWarehouse(dto: CreateWarehouseDto): Promise<unknown> {
    ...
    await this.activity.log('warehouse.created', {
  ...
  async updateWarehouse(dto: UpdateWarehouseDto): Promise<unknown> {
    ...
    await this.activity.log('warehouse.updated', {
  ...
  async createLocation(dto: CreateStorageLocationDto): Promise<unknown> {
    ...
    await this.activity.log('storage_location.created', {
  ...
  async updateLocation(dto: UpdateStorageLocationDto): Promise<unknown> {
    ...
    await this.activity.log('storage_location.updated', {
      storageLocationId: updated.id,
  ...
  async moveInventory(dto: MoveInventoryDto): Promise<unknown> {
    ...
    if (dto.toStorageLocationId && !target) {
      throw new NotFoundException('Target storage location not found');
    }

    const movementQuantity = dto.quantity ?? inventory.quantity;

    if (movementQuantity > inventory.quantity) {
      throw new BadRequestException('Movement quantity exceeds inventory quantity');
    }
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      const row = await tx.inventoryItem.update({
        ...
        data: {
          storageLocationId: target?.id ?? null,
          warehouseId: target?.warehouseId ?? null,
          storageLocation: target?.name ?? null,
        },
        include: {
          item: true,
          assignedUser: true,
    ...
    await this.activity.log('inventory.moved', {
      inventoryItemId: inventory.id,
      fromStorageLocationId: inventory.storageLocationId,
      toStorageLocationId: target?.id ?? null,
      quantity: movementQuantity,
      reason: dto.reason ?? null,
    });
  ...
  async warehouseSnapshot(): Promise<unknown[]> {
    ...
    return warehouses.map((warehouse) => {
      const inventoryItems = warehouse.locations.flatMap(
        (location) => location.inventoryItems,
      );
      ...
      return {
        ...
        units: inventoryItems.reduce((sum, item) => sum + item.quantity, 0),
        costBasis: inventoryItems.reduce((sum, item) => sum + item.totalCost, 0),
      };
    });
  }
}