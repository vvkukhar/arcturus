...
export class ProcurementService {
  ...
  async create(dto: CreatePurchaseOrderDto): Promise<unknown> {
    ...
    const actualPrice =
      dto.actualPrice != null ? toMoney(dto.actualPrice) : null;
    const plannedPrice =
      dto.plannedPrice != null ? toMoney(dto.plannedPrice) : null;
    const shippingPrice =
      dto.shippingPrice != null ? toMoney(dto.shippingPrice) : null;
    ...
    const created = await this.prisma.purchaseOrder.create({
      data: {
        ...
        targetSellPrice:
          dto.targetSellPrice != null ? toMoney(dto.targetSellPrice) : null,
    ...
    await this.activity.log('purchase_order.created', {
  ...
  async createFromWatchlist(watchlistItemId: string): Promise<unknown> {
    ...
    const existing = await this.prisma.purchaseOrder.findFirst({
      where: {
        watchlistItemId,
        status: {
          in: ['planned', 'approved', 'ordered', 'paid', 'received'],
        },
      },
    });
  ...
  async update(dto: UpdatePurchaseOrderDto): Promise<unknown> {
    ...
    const actualPrice =
      dto.actualPrice === undefined
        ...
        : dto.actualPrice === null
          ? null
          : toMoney(dto.actualPrice);

    const plannedPrice =
      dto.plannedPrice === undefined
        ? existing.plannedPrice
        : dto.plannedPrice === null
          ? null
          : toMoney(dto.plannedPrice);

    const shippingPrice =
      dto.shippingPrice === undefined
        ? existing.shippingPrice
        : dto.shippingPrice === null
          ? null
          : toMoney(dto.shippingPrice);
    ...
    const updated = await this.prisma.purchaseOrder.update({
      ...
      data: {
        ...
        targetSellPrice:
          dto.targetSellPrice === undefined
            ? undefined
            : dto.targetSellPrice === null
              ? null
              : toMoney(dto.targetSellPrice),
        quantity: dto.quantity,
        condition: dto.condition,
        sealed: dto.sealed,
        notes: dto.notes,
        purchasedAt:
          dto.status === 'ordered' || dto.status === 'paid'
    ...
    await this.activity.log('purchase_order.updated', {
  ...
  async receive(dto: ReceivePurchaseOrderDto): Promise<unknown> {
    ...
    if (dto.storageLocationId && !location) {
    ...
    if (cost == null || cost <= 0) {
      throw new BadRequestException('Purchase order has no valid cost');
    }
    ...
    await this.activity.log('purchase_order.received', {
  ...
  async stats(): Promise<unknown> {
    ...
    return {
      ...
      openCost: toMoney(
        open.reduce((sum, order) => sum + Number(order.totalCost ?? order.plannedPrice ?? 0), 0),
      ),
      receivedCost: toMoney(
        orders
